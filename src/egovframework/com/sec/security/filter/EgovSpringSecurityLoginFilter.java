	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		if (EgovUserDetailsHelper.getAuthenticatedUser() == null || requestURL.contains(loginProcessURL)) {
			...
			if (isRemotelyAuthenticated != null && isRemotelyAuthenticated.equals("true")) {
				...
				if (requestURL.contains(loginProcessURL)) {
					...
					try {
						...
						if (loginVO != null && loginVO.getId() != null && !loginVO.getId().equals("")) {
							...
							/** 최세원 2017-06-05 로그인이 성공했으나 비밀번호 5회 체크가 잘못되었다면 1시간 로그인 안되게 막음 */
							//1시간이 지났다면 초기화
							if(loginVO.getChkDt().equals("Y")){
								loginService.updateFailNull(loginVO);

							//실패카운트가 5회이고 1시간이 지나지 않았다면 ...
							}else if(Integer.parseInt(loginVO.getFailCnt()) > 4 && loginVO.getChkDt().equals("N")){
								httpRequest.setAttribute("message", "비밀번호 5회 오류로 1시간동안 접속이 제한됩니다.");
								RequestDispatcher dispatcher = httpRequest.getRequestDispatcher(loginURL);
								dispatcher.forward(httpRequest, httpResponse);
								return;
							}

							//2017.11.22 고객지원 - 비밀번호 변경 4개월 이후에는 로그인 안되도록 수정
							String timeCheck = loginService.findLastPwdChgTime(loginVO);
							if(("Y").equals(timeCheck)){
								httpRequest.setAttribute("message", "4개월 동안 비밀번호를 변경하지 않아 계정이 잠겼습니다.\\n관리자에게 [비밀번호 초기화]를 요청하세요.");
								RequestDispatcher dispatcher = httpRequest.getRequestDispatcher(loginURL);
								dispatcher.forward(httpRequest, httpResponse);
								return;
							}

							//한시간이 지나지 않고 5번 미만에 로그인이 되었다면 실패 시간 및 카운트를 다시 초기화
							loginService.updateFailNull(loginVO);
							...
							//사용자 입력 id로 DB 인증해서 비밀번호는 틀렸지만 ID가 있는지 체크후 있으면 실패 카운트와 시간을 업데이트
							loginVO.setId(httpRequest.getParameter("id"));
							loginVO = loginService.actionLoginIdCheck(loginVO);

							/** 최세원 2017-06-05 로그인 실패시 비밀번호 5회 체크가 잘못되었다면 1시간 로그인 안되게 막음 */
							if(loginVO.getChkDt().equals("Y")){
								//실패 카운트와 실패 시간을 초기화 한다.
								loginService.updateFailNull(loginVO);

								//실패 카운트 5회이고 1시간이 지나지 않았다면 오류
							}else if(Integer.parseInt(loginVO.getFailCnt()) > 4  && loginVO.getChkDt().equals("N")){
								httpRequest.setAttribute("message", "비밀번호 5회 오류로 1시간동안 접속이 제한됩니다.");
								RequestDispatcher dispatcher = httpRequest.getRequestDispatcher(loginURL);
								dispatcher.forward(httpRequest, httpResponse);
							...
							if(Integer.parseInt(loginVO.getFailCnt()) < 6){
								//5번까지는 실패 카운트와 실패 시간을 업데이트 한다.
								loginService.updateFail(loginVO);
							}

							//사용자 정보가 없는 경우 로그인 화면으로 redirect 시킴
							//httpRequest.setAttribute("message", egovMessageSource.getMessage("fail.common.login"));
							String failMsg = egovMessageSource.getMessage("fail.common.login");
							int failCnt = 0;
							if(loginVO.getFailCnt() != null){
								failCnt = Integer.parseInt(loginVO.getFailCnt()) + 1;
								failMsg = "비밀번호 " + failCnt + "회 오류(5회 오류시 1시간동안 접속이 제한됩니다)";
							}

							httpRequest.setAttribute("message", failMsg);
				...
				}else{//세션정보없이 시스템 접근시 로그인 화면으로
					LOGGER.error("with login.......");
					session.invalidate();//세션초기화
					RequestDispatcher dispatcher = httpRequest.getRequestDispatcher("/EgovContent.do");
					dispatcher.forward(httpRequest, httpResponse);
